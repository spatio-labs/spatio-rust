...
/// SheetListEnvelope : Fan-out response for `GET /v1/sheets`. `items` aggregates sheets across every connected account; `accounts` carries one row per contributing connection — including failures. 
#[derive(Clone, Default, Debug, PartialEq, Serialize, Deserialize)]
pub struct SheetListEnvelope {
    #[serde(rename = "items")]
    pub items: Vec<models::Sheet>,
    #[serde(rename = "accounts")]
    pub accounts: Vec<models::AccountStatus>,
}

impl SheetListEnvelope {
    ...
    pub fn new(items: Vec<models::Sheet>, accounts: Vec<models::AccountStatus>) -> SheetListEnvelope {
        SheetListEnvelope {
            items,
            accounts,
        }
