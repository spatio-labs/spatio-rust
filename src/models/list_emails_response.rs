...
/// ListEmailsResponse : List of emails across the selected accounts. `provider` is set on single-account calls; on fan-out it carries the value from the first contributing account (legacy behavior — clients should rely on the per-row `provider` field instead). 
...
impl ListEmailsResponse {
    ...
    pub fn new(total: i32, provider: String) -> ListEmailsResponse {
        ListEmailsResponse {
            emails: None,
            total,
