...
/// struct for typed errors of method [`get_navigation`]
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(untagged)]
pub enum GetNavigationError {
    Status401(models::ApiError),
    UnknownValue(serde_json::Value),
}


pub async fn get_navigation(configuration: &configuration::Configuration, ) -> Result<std::collections::HashMap<String, serde_json::Value>, Error<GetNavigationError>> {

    let uri_str = format!("{}/v1/navigation", configuration.base_path);
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        let content = resp.text().await?;
        let entity: Option<GetNavigationError> = serde_json::from_str(&content).ok();
