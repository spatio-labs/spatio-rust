...
/// struct for typed errors of method [`federated_search`]
...
pub async fn federated_search(configuration: &configuration::Configuration, federated_search_request: models::FederatedSearchRequest) -> Result<models::FederatedSearch200Response, Error<FederatedSearchError>> {
    // add a prefix to parameters to efficiently prevent name collisions
    let p_body_federated_search_request = federated_search_request;
    ...
    req_builder = req_builder.json(&p_body_federated_search_request);
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        match content_type {
            ...
            ContentType::Text => return Err(Error::from(serde_json::Error::custom("Received `text/plain` content type response that cannot be converted to `models::FederatedSearch200Response`"))),
            ContentType::Unsupported(unknown_type) => return Err(Error::from(serde_json::Error::custom(format!("Received `{unknown_type}` content type response that cannot be converted to `models::FederatedSearch200Response`")))),
        }
    } else {
        let content = resp.text().await?;
        let entity: Option<FederatedSearchError> = serde_json::from_str(&content).ok();
